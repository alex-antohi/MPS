...
def encode(file):
    ...
    df['confirmare contact cu o persoană infectată'] = df['confirmare contact cu o persoană infectată'].replace(
        np.nan, 0)
    ...
    model = OneVsRestClassifier(svm.SVC(class_weight='balanced', probability=True)).fit(X_antrenare, y_antrenare)
    ...
    output_string = 'Acuratete_v = ' + "{:.1f}".format(acuratete) + "\n"

    precizie = TP / (TP + FP)
    output_string += 'Precizie_v = ' + "{:.1f}".format(precizie) + "\n"
    ...
    output_string += "Matricea = " + str(mat_conf1) + "\n\n"
    ...
    acuratete = (TP + TN) / (TP + FP + TN + FN)
    output_string += 'Acuratete_t = ' + "{:.1f}".format(acuratete) + "\n"

    precizie = TP / (TP + FP)
    output_string += 'Precizie_t = ' + "{:.1f}".format(precizie) + "\n"
